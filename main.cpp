...
void createPointCloud();    // Update point cloud
void drawPointCloud(float); // Draw Point cloud
...
int main()
{
  ...
  pangolin::CreateWindowAndBind("Point Cloud", 640, 480);
  ...
  pangolin::Var<float> point_size("ui.Point Size", 1, 0.5, 5);
...
void createPointCloud()
{
  ...
  float d_step = 0.02;
  float d_max = 1.0;
  int steps = (d_max - d_min)/d_step;
  for (int i = 0; i < steps; i++)
  {
    vertices.push_back({i * d_step, 0, 0});
    colors.push_back({1, 0, 0}); // red

    vertices.push_back({0, i * d_step, 0});
    colors.push_back({0, 1, 0}); // green

    vertices.push_back({0, 0, i * d_step});
    colors.push_back({0, 0, 1}); // blue
  }
}

// Draw the point cloud
void drawPointCloud(float point_size)
{
  glMatrixMode(GL_MODELVIEW);

  glDisable(GL_LIGHTING); // Optional

  // Save state onto the matrix stack,
  glPushMatrix();

  glPointSize(point_size); // Point size for rendering point cloud
